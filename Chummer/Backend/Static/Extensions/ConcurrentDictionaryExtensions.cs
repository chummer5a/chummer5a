...
namespace Chummer
{
    public static class ConcurrentDictionaryExtensions
    {
        ...
        public static Task<TValue> GetOrAddAsync<TKey, TValue>(
            [NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key,
            Func<TKey, CancellationToken, Task<TValue>> valueFactory, CancellationToken token = default)
        {
            if (token.IsCancellationRequested)
                return Task.FromCanceled<TValue>(token);
            return dicThis.TryGetValue(key, out TValue objReturn) ? Task.FromResult(objReturn) : Inner();
            ...
            async Task<TValue> Inner()
            {
                TValue objNewValue = await valueFactory.Invoke(key, token).ConfigureAwait(false);
                TValue objInnerReturn = dicThis.GetOrAdd(key, objNewValue);
                if (!ReferenceEquals(objInnerReturn, objNewValue))
                {
                    if (objNewValue is IAsyncDisposable objDisposeAsync)
        ...
        public static Task<TValue> GetOrAddAsync<TKey, TArg, TValue>(
            [NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key,
            Func<TKey, TArg, CancellationToken, Task<TValue>> valueFactory, TArg factoryArgument, CancellationToken token = default)
        {
            if (token.IsCancellationRequested)
                return Task.FromCanceled<TValue>(token);
            return dicThis.TryGetValue(key, out TValue objReturn) ? Task.FromResult(objReturn) : Inner();
            ...
            async Task<TValue> Inner()
            {
                TValue objNewValue = await valueFactory.Invoke(key, factoryArgument, token).ConfigureAwait(false);
                TValue objInnerReturn = dicThis.GetOrAdd(key, objNewValue);
                if (!ReferenceEquals(objInnerReturn, objNewValue))
                {
                    if (objNewValue is IAsyncDisposable objDisposeAsync)
        ...
        public static Task<TValue> GetOrAddAsync<TKey, TArg, TValue>(
            [NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key,
            Func<TKey, TArg, CancellationToken, Task<TValue>> valueFactory, Task<TArg> factoryArgument, CancellationToken token = default)
        {
            if (token.IsCancellationRequested)
                return Task.FromCanceled<TValue>(token);
            return dicThis.TryGetValue(key, out TValue objReturn) ? Task.FromResult(objReturn) : Inner();
            ...
            async Task<TValue> Inner()
            {
                TValue objNewValue = await valueFactory.Invoke(key, await factoryArgument.ConfigureAwait(false), token)
                    .ConfigureAwait(false);
                TValue objInnerReturn = dicThis.GetOrAdd(key, objNewValue);
                if (!ReferenceEquals(objInnerReturn, objNewValue))
                {
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TArg, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, TArg, CancellationToken, Task<TValue>> addValueFactory,
                                                        Func<TKey, TValue, TArg, TValue> updateValueFactory, TArg factoryArgument, CancellationToken token = default)
        {
            while (true)
            {
                ...
                objReturn = await addValueFactory(key, factoryArgument, token).ConfigureAwait(false);
                token.ThrowIfCancellationRequested();
                objExistingValue = dicThis.GetOrAdd(key, objReturn);
                if (!ReferenceEquals(objExistingValue, objReturn))
                {
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TArg, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, TArg, Task<TValue>> addValueFactory,
                                                        Func<TKey, TValue, TArg, CancellationToken, TValue> updateValueFactory, TArg factoryArgument, CancellationToken token = default)
        {
            while (true)
            {
                token.ThrowIfCancellationRequested();
                ...
                while (dicThis.TryGetValue(key, out objExistingValue))
                {
                    objReturn = updateValueFactory(key, objExistingValue, factoryArgument, token);
                    token.ThrowIfCancellationRequested();
                    if (dicThis.TryUpdate(key, objReturn, objExistingValue))
                        return objReturn;
                }
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TArg, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, TArg, CancellationToken, TValue> addValueFactory,
                                                        Func<TKey, TValue, TArg, Task<TValue>> updateValueFactory, TArg factoryArgument, CancellationToken token = default)
        {
            while (true)
            {
                ...
                objReturn = addValueFactory(key, factoryArgument, token);
                token.ThrowIfCancellationRequested();
                objExistingValue = dicThis.GetOrAdd(key, objReturn);
                if (!ReferenceEquals(objExistingValue, objReturn))
                {
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TArg, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, TArg, TValue> addValueFactory,
                                                        Func<TKey, TValue, TArg, CancellationToken, Task<TValue>> updateValueFactory, TArg factoryArgument, CancellationToken token = default)
        {
            while (true)
            {
                token.ThrowIfCancellationRequested();
                ...
                while (dicThis.TryGetValue(key, out objExistingValue))
                {
                    objReturn = await updateValueFactory(key, objExistingValue, factoryArgument, token).ConfigureAwait(false);
                    token.ThrowIfCancellationRequested();
                    if (dicThis.TryUpdate(key, objReturn, objExistingValue))
                        return objReturn;
                }
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, CancellationToken, Task<TValue>> addValueFactory,
                                                        Func<TKey, TValue, TValue> updateValueFactory, CancellationToken token = default)
        {
            while (true)
            {
                ...
                objReturn = await addValueFactory(key, token).ConfigureAwait(false);
                token.ThrowIfCancellationRequested();
                objExistingValue = dicThis.GetOrAdd(key, objReturn);
                if (!ReferenceEquals(objExistingValue, objReturn))
                {
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, Task<TValue>> addValueFactory,
                                                        Func<TKey, TValue, CancellationToken, TValue> updateValueFactory, CancellationToken token = default)
        {
            while (true)
            {
                token.ThrowIfCancellationRequested();
                ...
                while (dicThis.TryGetValue(key, out objExistingValue))
                {
                    objReturn = updateValueFactory(key, objExistingValue, token);
                    token.ThrowIfCancellationRequested();
                    if (dicThis.TryUpdate(key, objReturn, objExistingValue))
                        return objReturn;
                }
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, CancellationToken, TValue> addValueFactory,
                                                        Func<TKey, TValue, Task<TValue>> updateValueFactory, CancellationToken token = default)
        {
            while (true)
            {
                ...
                objReturn = addValueFactory(key, token);
                token.ThrowIfCancellationRequested();
                objExistingValue = dicThis.GetOrAdd(key, objReturn);
                if (!ReferenceEquals(objExistingValue, objReturn))
                {
        ...
        public static async Task<TValue> AddOrUpdateAsync<TKey, TValue>([NotNull] this ConcurrentDictionary<TKey, TValue> dicThis, TKey key, Func<TKey, TValue> addValueFactory,
                                                        Func<TKey, TValue, CancellationToken, Task<TValue>> updateValueFactory, CancellationToken token = default)
        {
            while (true)
            {
                token.ThrowIfCancellationRequested();
                ...
                while (dicThis.TryGetValue(key, out objExistingValue))
                {
                    objReturn = await updateValueFactory(key, objExistingValue, token).ConfigureAwait(false);
                    token.ThrowIfCancellationRequested();
                    if (dicThis.TryUpdate(key, objReturn, objExistingValue))
                        return objReturn;
                }
