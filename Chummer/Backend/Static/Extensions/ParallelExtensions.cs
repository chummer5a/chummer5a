...
namespace Chummer
{
    ...
    {
        /// <summary>
        /// Syntactic sugar to process a batch of asynchronous method calls in parallel while respecting the max parallel batch size we have set in Utils
        /// </summary>
        ...
        public static async Task<List<TResult>> ForEachAsync<TSource, TResult>(IAsyncEnumerable<TSource> lstItems, Func<TSource, TResult> funcCodeToRun, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        token.ThrowIfCancellationRequested();
                        aobjTaskBuffer[i++] = Task.Run(() => funcCodeToRun(objEnumerator.Current));
                        if (i == Utils.MaxParallelBatchSize)
                        {
                            lstReturn.AddRange(await Task.WhenAll(aobjTaskBuffer).ConfigureAwait(false));
                            i = 0;
