...
namespace Chummer.Backend.Skills
{
    ...
    partial class Skill : IHasInternalId
    {
        ...
        {
            string strReturn = $"{DisplayNameMethod(GlobalOptions.Language)}{space}({pool.ToString(GlobalOptions.Instance.CultureInfo)})";
            ...
            if (HasSpecialization(validSpec) && !string.IsNullOrWhiteSpace(validSpec))
                strReturn +=
                    ...
                    $"{DisplayCategory(GlobalOptions.Language)}{space}{'('}{2.ToString(GlobalOptions.Instance.CultureInfo)}{')'}";
            return strReturn;
        }
    }
}
