    ...
    public sealed class SkillsSection : INotifyMultiplePropertyChangedAsync, IHasLockObject
    {
        ...
        private async Task KnowledgeSkillsOnListChanged(object sender, ListChangedEventArgs e, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                switch (e.ListChangedType)
                {
                    case ListChangedType.Reset:
                    {
                        await KnowledgeSkills.ForEachAsync(objKnoSkill => objKnoSkill.AddPropertyChangedAsync(OnKnowledgeSkillPropertyChanged, token), token: token).ConfigureAwait(false);
                    ...
                    case ListChangedType.ItemAdded:
                    {
                        ...
                        if (objKnoSkill != null)
                        {
                            await objKnoSkill.AddPropertyChangedAsync(OnKnowledgeSkillPropertyChanged, token)
                                .ConfigureAwait(false);
                        }
                ...
                try
                {
                           ...
                           Timekeeper.StartSyncron("load_char_skills_skillnode", parentActivity))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                await KnowledgeSkills.ForEachAsync(objKnoSkill => objKnoSkill.AddPropertyChangedAsync(OnKnowledgeSkillPropertyChanged, token), token).ConfigureAwait(false);
                            }
        ...
        public async Task<int> GetKnowledgeSkillPointsAsync(CancellationToken token = default)
        {
            using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                ...
                IAsyncDisposable objLocker = await _objCachedKnowledgePointsLock.EnterUpgradeableReadLockAsync(token).ConfigureAwait(false);
                try
                {
                    token.ThrowIfCancellationRequested();
                    if (_intCachedKnowledgePoints != int.MinValue)
