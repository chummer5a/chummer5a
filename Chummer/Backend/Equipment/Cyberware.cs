        ...
        IHasWirelessBonus, ICanBlackMarketDiscount, IHasLockObject
    {
                        ...
                        case NotifyCollectionChangedAction.Remove:
                        {
                            ...
                            if (lstOldItems != null)
                            {
                                foreach (Cyberware objOldItem in lstOldItems)
                                {
                                    try
                                    {
                                        await using (await objOldItem.LockObject.EnterUpgradeableReadLockAsync(token).ConfigureAwait(false))
                                        {
                                            token.ThrowIfCancellationRequested();
                           ...
                           out HashSet<string> setAttributesToRefresh))
                {
                                   ...
                                   dicChangedProperties))
                    {
                        try
                        {
                            ...
                            {
                                await foreach (CharacterAttrib objCharacterAttrib in
                                         _objCharacter.GetAllAttributesAsync(strAbbrev, token: token).ConfigureAwait(false))
                                {
                            ...
                            List<Cyberware> lstPairableCyberwares = blnSync
                                ...
                                : await _objCharacter.Cyberware.DeepWhereAsync(x => x.GetChildrenAsync(token),
                                    async x => x != this && IncludePair.Contains(x.Name) && x.Extra == Extra &&
                                               await x.GetIsModularCurrentlyEquippedAsync(token).ConfigureAwait(false), token: token).ToListAsync(token).ConfigureAwait(false);
            ...
            CancellationToken token = default)
        {
            ...
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                token.ThrowIfCancellationRequested();
                // <cyberware>
                await using (await objWriter.StartElementAsync("cyberware", token: token).ConfigureAwait(false))
                {
                    ...
                    {
                        // <gears>
                        await using (await objWriter.StartElementAsync("gears", token: token).ConfigureAwait(false))
                        {
                    ...
                    // <children>
                    await using (await objWriter.StartElementAsync("children", token: token).ConfigureAwait(false))
                    {
        ...
        public async Task<bool> GetIsModularCurrentlyEquippedAsync(CancellationToken token = default)
        {
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                ...
                while (objCurrentParent != null)
                {
                    await using (await objCurrentParent.LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
                    {
                        token.ThrowIfCancellationRequested();
        ...
        public async Task<bool> GetCanRemoveThroughImprovementsAsync(CancellationToken token = default)
        {
            ...
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                ...
                {
                    token.ThrowIfCancellationRequested();
                    await using (await objGrandparent.LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
                    {
                        token.ThrowIfCancellationRequested();
                        Cyberware objParent = objGrandparent;
                                                       ...
                                                       && await _objParent.GetParentVehicleAsync(token).ConfigureAwait(false) == null)
                                {
                                                ...
                                                .Contains(await _objParent.GetLimbSlotAsync(token).ConfigureAwait(false)))
                                    {
                                                 ...
                                                 s_AttributeAffectingCyberwares)
                                        {
                                            if (!kvpToCheck.Value.Contains(Name))
                                                continue;
                                            await foreach (CharacterAttrib objCharacterAttrib in _objCharacter.GetAllAttributesAsync(
                                ...
                                if (PairBonus != null)
                                {
                                    ...
                                    List<Cyberware> lstPairableCyberwares = await (await _objCharacter.GetCyberwareAsync(token).ConfigureAwait(false))
                                        .DeepWhereAsync(
                                            ...
                                            async x => x != this && IncludePair.Contains(x.Name) && x.Extra == Extra &&
                                                       await x.GetIsModularCurrentlyEquippedAsync(token)
                                                           .ConfigureAwait(false), token).ToListAsync(token).ConfigureAwait(false);
                                    int intCount = lstPairableCyberwares.Count;
                                    // Need to use slightly different logic if this cyberware has a location (Left or Right) and only pairs with itself because Lefts can only be paired with Rights and Rights only with Lefts
                                    if (!string.IsNullOrEmpty(Location) && IncludePair.All(x => x == Name))
                                    {
        ...
        public async Task<Cyberware> GetTopMostParentAsync(CancellationToken token = default)
        {
            ...
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                ...
                while (objParent != null)
                {
                    await using (await objParent.LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
                    {
                        token.ThrowIfCancellationRequested();
                        objReturn = objParent;
        ...
        CancellationToken token = default)
        {
            ...
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                ...
                TaggedObservableCollection<Gear> gearChildrenToCheck = await GetGearChildrenAsync(token).ConfigureAwait(false);
        ...
        public async Task<bool> AllowPasteObject(object input, CancellationToken token = default)
        {
            ...
            await using (await LockObject.EnterReadLockAsync(token).ConfigureAwait(false))
            {
                           ...
                           out HashSet<string> setHasMounts))
                {
                    ...
                    await foreach (Cyberware objLoopCyberware in (await GetChildrenAsync(token).ConfigureAwait(false)).DeepWhereAsync(
