...
namespace Chummer.Backend.Equipment
{
    ...
    public sealed class Vehicle : IHasInternalId, IHasName, IHasSourceId, IHasXmlDataNode, IHasMatrixAttributes, IHasNotes, ICanSell, IHasCustomName, IHasPhysicalConditionMonitor, IHasLocation, IHasSource, ICanSort, IHasGear, IHasStolenProperty, ICanPaste, ICanBlackMarketDiscount, IDisposable, IAsyncDisposable
    {
                                     ...
                                     CancellationToken token = default)
        {
            ...
            await Mods.ForEachAsync((x, t) => x.Print(objWriter, objCulture, strLanguageToPrint, t), token).ConfigureAwait(false);
            await WeaponMounts.ForEachAsync((x, t) => x.Print(objWriter, objCulture, strLanguageToPrint, t), token).ConfigureAwait(false);
            await objWriter.WriteEndElementAsync().ConfigureAwait(false);
            await objWriter.WriteStartElementAsync("gears", token: token).ConfigureAwait(false);
            await GearChildren.ForEachAsync((x, t) => x.Print(objWriter, objCulture, strLanguageToPrint, t), token).ConfigureAwait(false);
            await objWriter.WriteEndElementAsync().ConfigureAwait(false);
            await objWriter.WriteStartElementAsync("weapons", token: token).ConfigureAwait(false);
            await Weapons.ForEachAsync((x, t) => x.Print(objWriter, objCulture, strLanguageToPrint, t), token).ConfigureAwait(false);
            await objWriter.WriteEndElementAsync().ConfigureAwait(false);
        ...
        public async Task<int> GetPilotAsync(VehicleMod objExcludeMod, CancellationToken token = default)
        {
            ...
            try
            {
                token.ThrowIfCancellationRequested();
                int intReturn = BasePilot;
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    if (!objMod.IncludedInVehicle && objMod.Equipped && !ReferenceEquals(objMod, objExcludeMod))
                    {
                        string strBonusPilot = objMod.WirelessOn
                            ? objMod.WirelessBonus?["pilot"]?.InnerTextViaPool(t) ?? objMod.Bonus?["pilot"]?.InnerTextViaPool(t)
                            : objMod.Bonus?["pilot"]?.InnerTextViaPool(t);
                        intReturn = Math.Max(
                            await ParseBonusAsync(strBonusPilot, objMod, BasePilot, "Pilot", false, t)
                                .ConfigureAwait(false),
                            intReturn);
                    }
                }, token).ConfigureAwait(false);
        ...
        public async Task<int> GetCalculatedSensorAsync(VehicleMod objExcludeMod, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    ...
                    intTotalSensor = Math.Max(intTotalSensor,
                        await ParseBonusAsync(strLoop, objMod, intTotalSensor, "Sensor", false, t)
                            .ConfigureAwait(false));
                    if (!objMod.WirelessOn || objMod.WirelessBonus == null)
                        return;
        ...
        private static int ParseBonus(string strBonus, VehicleMod objMod, int intTotalRating, string strReplaceRating, bool blnBonus = true, CancellationToken token = default)
        {
            token.ThrowIfCancellationRequested();
            if (strBonus.DoesNeedXPathProcessingToBeConvertedToNumber(out decimal decValue))
            {
                ...
                if ((chrFirstCharacter == '+' || chrFirstCharacter == '-') && blnBonus)
                {
                    ...
                    return objMod.ProcessRatingString(strBonus.Replace("{" + strReplaceRating + "}", strReplaceValue).Replace(strReplaceRating, strReplaceValue), () => objMod.Rating, token);
                }
                if (chrFirstCharacter != '+' && chrFirstCharacter != '-' && !blnBonus)
                {
                    // If the bonus is determined by the existing number, evaluate the expression.
        ...
        public async Task<int> GetTotalSeatsAsync(VehicleMod objExcludeMod, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    ...
                    string strBonusSeats = objMod.WirelessOn
                        ? objMod.WirelessBonus?["seats"]?.InnerTextViaPool(t) ?? objMod.Bonus?["seats"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["seats"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonusSeats))
                        intTotalSeats =
                            Math.Max(
        ...
        public async Task<string> GetTotalSpeedAsync(VehicleMod objExcludeMod, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    ...
                    string strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["speed"]?.InnerTextViaPool(t) ?? objMod.Bonus?["speed"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["speed"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonus))
                    ...
                    strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["offroadspeed"]?.InnerTextViaPool(t) ?? objMod.Bonus?["offroadspeed"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["offroadspeed"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonus))
                        intBaseOffroadSpeed =
                            Math.Max(
                                await ParseBonusAsync(strBonus, objMod, OffroadSpeed,
                                        "OffroadSpeed", false, t)
                                    .ConfigureAwait(false),
                                intTotalSpeed);
                    if (IsDrone && await objSettings.GetDroneModsAsync(t).ConfigureAwait(false))
                    {
                        strBonus = objMod.Bonus?["armor"]?.InnerTextViaPool(t);
                        if (!string.IsNullOrEmpty(strBonus))
                            intTotalArmor =
                                Math.Max(
                        ...
                        if (objMod.WirelessOn && objMod.WirelessBonus != null)
                        {
                            strBonus = objMod.WirelessBonus["armor"]?.InnerTextViaPool(t);
                            if (!string.IsNullOrEmpty(strBonus))
                                intTotalArmor =
                                    Math.Max(
                                        await ParseBonusAsync(strBonus, objMod,
                                                intTotalArmor, "Armor", false, t)
                                            .ConfigureAwait(false),
                                        intTotalArmor);
                        }
                    }
            ...
            try
            {
                ...
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    if (objMod.IncludedInVehicle || !objMod.Equipped || ReferenceEquals(objMod, objExcludeMod))
                        return;
                    string strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["accel"]?.InnerTextViaPool(t) ?? objMod.Bonus?["accel"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["accel"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonus))
                        intTotalAccel = Math.Max(
                            await ParseBonusAsync(strBonus, objMod, Accel, "Accel", false, t)
                                .ConfigureAwait(false),
                            intTotalAccel);

                    strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["offroadaccel"]?.InnerTextViaPool(t) ?? objMod.Bonus?["offroadaccel"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["offroadaccel"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonus))
                        intBaseOffroadAccel =
                            Math.Max(
                    ...
                    if (IsDrone && await objSettings.GetDroneModsAsync(t).ConfigureAwait(false))
                    {
                        ...
                        if (!string.IsNullOrEmpty(strBonus))
                            intTotalArmor =
                                Math.Max(
                                    await ParseBonusAsync(strBonus, objMod, intTotalArmor, "Armor", false, t)
                                        .ConfigureAwait(false),
                                    intTotalArmor);
                        if (objMod.WirelessOn && objMod.WirelessBonus != null)
                        {
        ...
        /// Total Handling of the Vehicle including Modifications.
        /// </summary>
        public string GetTotalHandling(VehicleMod objExcludeMod = null, CancellationToken token = default)
        {
            using (_objCharacter.LockObject.EnterReadLock(token))
            {
                ...
                foreach (VehicleMod objMod in Mods)
                {
                    ...
                    if (!string.IsNullOrEmpty(strBonus))
                        intBaseHandling = Math.Max(ParseBonus(strBonus, objMod, Handling, "Handling", false, token: token),
                            intBaseHandling);
                    ...
                    if (!string.IsNullOrEmpty(strBonus))
                        intBaseOffroadHandling =
                            Math.Max(ParseBonus(strBonus, objMod, OffroadHandling, "OffroadHandling", false, token: token),
                                intBaseOffroadHandling);
                    if (IsDrone && _objCharacter.Settings.DroneMods)
                    {
                        ...
                        if (!string.IsNullOrEmpty(strBonus))
                            intTotalArmor = Math.Max(
                                ParseBonus(strBonus, objMod, intTotalArmor, "Armor", false, token: token),
                                intTotalArmor);
                        if (objMod.WirelessOn && objMod.WirelessBonus != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(strBonus))
                                intTotalArmor =
                                    Math.Max(ParseBonus(strBonus, objMod, intTotalArmor, "Armor", false, token: token),
                                        intTotalArmor);
                        }
                    }
                }
                ...
                foreach (VehicleMod objMod in Mods)
                {
                    ...
                    if (objMod.Bonus != null)
                    {
                        intTotalBonusHandling += ParseBonus(objMod.Bonus["handling"]?.InnerTextViaPool(token), objMod,
                            intBaseOffroadHandling, "Handling", token: token);
                        intTotalBonusOffroadHandling += ParseBonus(objMod.Bonus["offroadhandling"]?.InnerTextViaPool(token),
                            objMod, intBaseOffroadHandling, "OffroadHandling", token: token);
                        if (IsDrone && _objCharacter.Settings.DroneMods)
                            intModArmor += ParseBonus(objMod.Bonus["armor"]?.InnerTextViaPool(token), objMod,
                                intTotalArmor, "Armor", token: token);
                    }

                    if (objMod.WirelessOn && objMod.WirelessBonus != null)
                    {
                        intTotalBonusHandling += ParseBonus(objMod.WirelessBonus["handling"]?.InnerTextViaPool(token),
                            objMod, intBaseOffroadHandling, "Handling", token: token);
                        intTotalBonusOffroadHandling +=
                            ParseBonus(objMod.WirelessBonus["offroadhandling"]?.InnerTextViaPool(token), objMod,
                                intBaseOffroadHandling, "OffroadHandling", token: token);
                        if (IsDrone && _objCharacter.Settings.DroneMods)
                            intModArmor += ParseBonus(objMod.WirelessBonus["armor"]?.InnerTextViaPool(token), objMod,
                                intTotalArmor, "Armor", token: token);
                    }
                }
            ...
            try
            {
                ...
                await Mods.ForEachAsync(async (objMod, t) =>
                {
                    if (objMod.IncludedInVehicle || !objMod.Equipped || ReferenceEquals(objMod, objExcludeMod))
                        return;
                    string strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["handling"]?.InnerTextViaPool(t) ?? objMod.Bonus?["handling"]?.InnerTextViaPool(t)
                        : objMod.Bonus?["handling"]?.InnerTextViaPool(t);
                    if (!string.IsNullOrEmpty(strBonus))
                        intBaseHandling =
                            Math.Max(
                                await ParseBonusAsync(strBonus, objMod, Handling, "Handling", false, t)
                                    .ConfigureAwait(false),
                                intBaseHandling);

                    strBonus = objMod.WirelessOn
                        ? objMod.WirelessBonus?["offroadhandling"]?.InnerTextViaPool(t) ??
                        ...
                        intBaseOffroadHandling =
                            Math.Max(
                                await ParseBonusAsync(strBonus, objMod, OffroadHandling, "OffroadHandling",
                                    false,
                                    t).ConfigureAwait(false), intBaseOffroadHandling);
                    if (IsDrone && await objSettings.GetDroneModsAsync(t).ConfigureAwait(false))
                    {
                        strBonus = objMod.Bonus?["armor"]?.InnerTextViaPool(t);
                        if (!string.IsNullOrEmpty(strBonus))
        ...
        public async Task<TreeNode> CreateTreeNode(ContextMenuStrip cmsVehicle, ContextMenuStrip cmsVehicleLocation, ContextMenuStrip cmsVehicleWeapon, ContextMenuStrip cmsWeaponAccessory, ContextMenuStrip cmsWeaponAccessoryGear, ContextMenuStrip cmsVehicleGear, ContextMenuStrip cmsVehicleWeaponMount, ContextMenuStrip cmsCyberware, ContextMenuStrip cmsCyberwareGear, CancellationToken token = default)
        {
            ...
            await Locations
                .ForEachAsync(
                    async (objLocation, t) =>
                        lstChildNodes.Add(await objLocation.CreateTreeNode(cmsVehicleLocation, t)
                            .ConfigureAwait(false)), token).ConfigureAwait(false);
            ...
            if (await WeaponMounts.GetCountAsync(token).ConfigureAwait(false) > 0)
            {
                ...
                // Weapon Mounts
                await WeaponMounts.ForEachAsync(async (objWeaponMount, t) =>
                {
                    TreeNode objLoopNode = await objWeaponMount.CreateTreeNode(cmsVehicleWeaponMount, cmsVehicleWeapon,
                        cmsWeaponAccessory, cmsWeaponAccessoryGear, cmsCyberware, cmsCyberwareGear, cmsVehicle, t).ConfigureAwait(false);
                    if (objLoopNode != null)
                    {
            ...
            await Weapons.ForEachAsync(async (objWeapon, t) =>
            {
                TreeNode objLoopNode = await objWeapon.CreateTreeNode(cmsVehicleWeapon, cmsWeaponAccessory,
                    cmsWeaponAccessoryGear, t).ConfigureAwait(false);
                if (objLoopNode != null)
                {
            ...
            {
                TreeNode objLoopNode = await objGear.CreateTreeNode(cmsVehicleGear, null, t).ConfigureAwait(false);
                if (objLoopNode != null)
                {
                    TreeNode objParent = objNode;
        ...
        public string ProcessAttributesInXPath(string strInput, VehicleMod objExcludeMod = null, WeaponMount objExcludeMount = null, IReadOnlyDictionary<string, int> dicValueOverrides = null, CancellationToken token = default)
        {
            token.ThrowIfCancellationRequested();
            if (string.IsNullOrEmpty(strInput))
                return string.Empty;
            ...
            using (new FetchSafelyFromObjectPool<StringBuilder>(Utils.StringBuilderPool, out StringBuilder sbdInput))
            {
                ...
                ProcessAttributesInXPath(sbdInput, strInput, objExcludeMod, objExcludeMount, dicValueOverrides, token);
                return sbdInput.ToString();
            }
        }

        public void ProcessAttributesInXPath(StringBuilder sbdInput, string strOriginal = "", VehicleMod objExcludeMod = null, WeaponMount objExcludeMount = null, IReadOnlyDictionary<string, int> dicValueOverrides = null, CancellationToken token = default)
        {
            token.ThrowIfCancellationRequested();
            ...
            sbdInput.CheapReplace(strOriginal, "Slots",
                () => strOwnSlots.Value);
            _objCharacter.ProcessAttributesInXPath(sbdInput, strOriginal, dicValueOverrides, token);
        }
