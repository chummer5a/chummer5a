...
namespace Chummer.Backend.Equipment
{
	...
	public class Vehicle
	{
		...
		public int DroneModSlots
		{
			get
			{
				return _lstVehicleMods.Where(objMod => !objMod.IncludedInVehicle && objMod.Installed).Where(objMod => objMod.CalculatedSlots < 0).Aggregate(_intDroneModSlots, (current, objMod) => current - objMod.CalculatedSlots);
			}
		}
		...
		public int TotalBody
		{
			get
			{
				...
				foreach (VehicleMod objMod in _lstVehicleMods.Where(objMod => !objMod.IncludedInVehicle && objMod.Installed && objMod.Bonus != null).Where(objMod => objMod.Bonus.InnerXml.Contains("<body>")))
				{
