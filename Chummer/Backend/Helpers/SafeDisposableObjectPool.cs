...
{
    /// <summary>
    /// A slimmed-down and simplified version of DisposableObjectPool that should have fewer weird behaviors than the Microsoft-provided DisposableObjectPool.
    /// Unlike Microsoft-provided DisposableObjectPool, the pool is set up in a way that always tracks garbage collections and should guarantee that it's impossible
    ...
    public sealed class SafeDisposableObjectPool<T> : IDisposable where T : class, IDisposable
    {
        ...
        /// <summary>
        /// Create a simplified version of an object pool that is for disposable objects.
        ...
        /// <param name="funcConstructor">Delegate to the constructor to be used for all objects in the pool.</param>
        /// <param name="actionRunOnReturn">Optional delegate to function to run when an object is returned to the pool, including if it's forcibly returned should the pool be disposed without it.</param>
        public SafeDisposableObjectPool(int intPoolSize, Func<T> funcConstructor, Action<T> actionRunOnReturn = null)
        {
        ...
        public void Return(ref T objToReturn)
        {
            ...
            T objLocal = Interlocked.Exchange(ref objToReturn, null) ?? throw new ArgumentNullException(nameof(objToReturn));
        ...
        public void Dispose()
        {
            T[] aobjSwappablePool = Interlocked.Exchange(ref _aobjSwappablePool, null);
            if (aobjSwappablePool == null)
                return;
            for (int i = 0; i < _aobjSwappablePool.Length; ++i)
            {
                if (Interlocked.Exchange(ref aobjSwappablePool[i], null) == null && _actionRunOnReturn != null)
                {
                    // Run _actionRunOnReturn because this is an object that has not been returned to the pool
                    T objTemp = Interlocked.Exchange(ref _aobjStoragePool[i], null);
                    if (objTemp != null)
                    {
                        _actionRunOnReturn.Invoke(objTemp);
                        objTemp.Dispose();
                    }
                }
                else
                    Interlocked.Exchange(ref _aobjStoragePool[i], null)?.Dispose();
            }
        }
    }
}
