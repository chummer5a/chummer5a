    ...
    public class Metamagic : IHasInternalId, IHasName, IHasXmlDataNode, IHasNotes, ICanRemove, IHasSource
    {
        ...
        public async ValueTask Print(XmlWriter objWriter, CultureInfo objCulture, string strLanguageToPrint)
        {
            if (objWriter == null)
                return;
            // <metamagic>
            XmlElementWriteHelper objBaseElement = await objWriter.StartElementAsync("metamagic").ConfigureAwait(false);
            try
            {
                ...
                await objWriter.WriteElementStringAsync("improvementsource", _eImprovementSource.ToString()).ConfigureAwait(false);
