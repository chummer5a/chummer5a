...
namespace Chummer
{
    ...
    public class Art : IHasInternalId, IHasName, IHasXmlNode, IHasNotes, ICanRemove, IHasSource
    {
        ...
        public void Create(XmlNode objXmlArtNode, Improvement.ImprovementSource objSource)
        {
            ...
            if (_nodBonus != null)
            {
                if (!ImprovementManager.CreateImprovements(_objCharacter, objSource, _guiID.ToString("D"), _nodBonus, true, 1, DisplayNameShort(GlobalOptions.Instance.Language)))
                {
                    _guiID = Guid.Empty;
                    return;
                }
        ...
        public TreeNode CreateTreeNode(ContextMenuStrip cmsArt, bool blnAddCategory = false)
        {
            ...
            if (blnAddCategory)
                strText = LanguageManager.GetString("Label_Art", GlobalOptions.Instance.Language) + LanguageManager.GetString("String_Space", GlobalOptions.Instance.Language) + strText;
        ...
        public bool Remove(Character character, bool blnConfirmDelete = true)
        {
            ...
            if (blnConfirmDelete)
            {
                if (!character.ConfirmDelete(LanguageManager.GetString("Message_DeleteArt", GlobalOptions.Instance.Language)))
                    return false;
            }
