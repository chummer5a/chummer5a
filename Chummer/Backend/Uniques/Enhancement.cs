    ...
    public class Enhancement : IHasInternalId, IHasName, IHasXmlDataNode, IHasNotes, ICanRemove, IHasSource
    {
        ...
        public async ValueTask Print(XmlWriter objWriter, string strLanguageToPrint)
        {
            if (objWriter == null)
                return;
            // <enhancement>
            XmlElementWriteHelper objBaseElement = await objWriter.StartElementAsync("enhancement").ConfigureAwait(false);
            try
            {
                ...
                await objWriter.WriteElementStringAsync("improvementsource", SourceType.ToString()).ConfigureAwait(false);
