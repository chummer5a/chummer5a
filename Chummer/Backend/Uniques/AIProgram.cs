    ...
    public class AIProgram : IHasInternalId, IHasName, IHasXmlNode, IHasNotes, ICanRemove, IHasSource
    {
        ...
        public void Create(XmlNode objXmlProgramNode, string strExtra = "", bool boolCanDelete = true)
        {
            ...
            if (string.IsNullOrEmpty(Notes))
            {
                ...
                if (objXmlProgramNode.TryGetStringFieldQuickly("nameonpage", ref strNameOnPage) &&
                ...
                {
                    string strTranslatedNameOnPage = DisplayName;

                    // don't check again it is not translated
                    if (strTranslatedNameOnPage != _strName)
                    {
                        // if we found <altnameonpage>, and is not empty and not the same as english we must use that instead
                        if (objXmlProgramNode.TryGetStringFieldQuickly("altnameonpage", ref strNameOnPage)
