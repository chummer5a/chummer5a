    ...
    public class MartialArtTechnique : IHasInternalId, IHasName, IHasXmlDataNode, IHasNotes, ICanRemove, IHasSource
    {
        ...
        public async ValueTask Print(XmlWriter objWriter, string strLanguageToPrint)
        {
            if (objWriter == null)
                return;
            // <martialarttechnique>
            XmlElementWriteHelper objBaseElement = await objWriter.StartElementAsync("martialarttechnique").ConfigureAwait(false);
            try
            {
                ...
                await objWriter.WriteElementStringAsync("name", await DisplayNameAsync(strLanguageToPrint).ConfigureAwait(false)).ConfigureAwait(false);
                ...
                await objWriter.WriteElementStringAsync("source", Source).ConfigureAwait(false);
