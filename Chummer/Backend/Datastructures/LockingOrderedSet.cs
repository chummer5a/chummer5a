...
{
    [Serializable]
    public sealed class LockingOrderedSet<T> : IAsyncSet<T>, IAsyncList<T>, IAsyncReadOnlyList<T>, IAsyncProducerConsumerCollection<T>, ISerializable, IDeserializationCallback, IHasLockObject, IEnumerableWithAsyncAndSideEffects<T>
    {
        ...
        public Task<IEnumerator<T>> GetEnumeratorAsync(CancellationToken token = default)
        {
            ...
            async Task<IEnumerator<T>> Inner(Task<LockingEnumerator<T>> tskInner)
            {
                ...
                objResult.SetEnumerator(_lstOrderedData.GetEnumerator());
                return objResult;
            }
        }
