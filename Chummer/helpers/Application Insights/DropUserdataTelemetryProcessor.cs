...
namespace Chummer
{
    public class DropUserdataTelemetryProcessor : ITelemetryProcessor
    {
        ...
        {
            ModifyItem(item);
            if (GlobalOptions.Instance.UseLoggingApplicationInsights == ProgramOptions.UseAILogging.Trace)
            {
            ...
            if (GlobalOptions.Instance.UseLoggingApplicationInsights >= ProgramOptions.UseAILogging.Info)
            {
