    ...
    public partial class frmSelectDrug : Form
    {
        ...
        private void lstDrug_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (xmlDrug != null)
            {
                ...
                chkBlackMarketDiscount.Checked = _objCharacter.BlackMarketDiscount && _setBlackMarketMaps.Contains(xmlDrug.SelectSingleNode("category")?.Value);
        ...
        private void UpdateDrugInfo()
        {
            ...
            if (!string.IsNullOrEmpty(strAvail))
            {
                ...
                strAvailExpr = strAvailExpr.CheapReplace("MinRating", () => nudRating.Minimum.ToString(GlobalOptions.Instance.InvariantCultureInfo))
            ...
            {
                lblAvail.Text = 0.ToString(GlobalOptions.Instance.CultureInfo);
            }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(strCost))
                {
                    ...
                    {
                        strCost = strCost.CheapReplace("MinRating", () => nudRating.Minimum.ToString(GlobalOptions.Instance.InvariantCultureInfo))
                            .CheapReplace("Rating", () => nudRating.Value.ToString(GlobalOptions.Instance.InvariantCultureInfo));

                        object objProcess = CommonFunctions.EvaluateInvariantXPath(strCost, out bool blnIsSuccess);
                        if (blnIsSuccess)
                        {
                            decItemCost = Convert.ToDecimal(objProcess, GlobalOptions.Instance.InvariantCultureInfo) * _decCostMultiplier;
