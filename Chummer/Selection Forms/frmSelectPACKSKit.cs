...
namespace Chummer
{
    public partial class frmSelectPACKSKit : Form
    {
        ...
        public frmSelectPACKSKit(Character objCharacter)
        {
            ...
            _xmlBaseChummerNode = XmlManager.Load("packs.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlGearsBaseGearsNode = XmlManager.Load("gear.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer/gears");
            _xmlWeaponsBaseChummerNode = XmlManager.Load("weapons.xml", _objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlArmorBaseChummerNode = XmlManager.Load("armor.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlQualitiesBaseQualitiesNode = XmlManager.Load("qualities.xml", _objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer/qualities");
            _xmlSkillsBaseChummerNode = XmlManager.Load("skills.xml", _objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlSpellsBaseSpellsNode = XmlManager.Load("spells.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer/spells");
            _xmlComplexFormsBaseChummerNode = XmlManager.Load("complexforms.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlVehiclesBaseChummerNode = XmlManager.Load("vehicles.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlBiowareBaseChummerNode = XmlManager.Load("bioware.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlCyberwareBaseChummerNode = XmlManager.Load("cyberware.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
            _xmlPowersBasePowersNode = XmlManager.Load("powers.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer/powers");
            _xmlMartialArtsBaseChummerNode = XmlManager.Load("martialarts.xml", objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
        }
                ...
                switch (objXmlItem.Name)
                {
                    case "attributes":
                        ...
                        foreach (XPathNavigator objXmlAttribute in objXmlItem.SelectChildren(XPathNodeType.Element))
                        {
                            ...
                            {
                                Text = LanguageManager.GetString("String_Attribute" + strNameUpper + "Short") + strSpaceCharacter + (Convert.ToInt32(objXmlAttribute.Value, GlobalOptions.InvariantCultureInfo) - (6 - _objCharacter.GetAttribute(strNameUpper).MetatypeMaximum)).ToString(GlobalOptions.CultureInfo)
                            };
                    ...
                    case "qualities":
                        ...
                        foreach (XPathNavigator objXmlQuality in objXmlItem.Select("positive/quality"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(strSelect))
                                objChild.Text += strSpaceCharacter + '('+ LanguageManager.TranslateExtra(strSelect)+ ')';
                            objParent.Nodes.Add(objChild);
                            objParent.Expand();
                        }
                    ...
                    case "nuyenbp":
                        ...
                        {
                            Text = LanguageManager.GetString("String_SelectPACKSKit_StartingNuyenBP") + strSpaceCharacter + objXmlItem.Value
                        };
                    ...
                    case "skills":
                        ...
                        foreach (XPathNavigator objXmlSkill in objXmlItem.Select("skillgroup"))
                        {
                            ...
                            objChild.Text += strSpaceCharacter + LanguageManager.GetString("String_SelectPACKSKit_Group")+ strSpaceCharacter + objXmlSkill.SelectSingleNode("rating")?.Value;
                    ...
                    case "selectmartialart":
                        {
                            ...
                            {
                                Text = strSelect + strSpaceCharacter + LanguageManager.GetString("String_Rating") + strSpaceCharacter + intRating.ToString(GlobalOptions.CultureInfo)
                            };
                        ...
                        foreach (XPathNavigator objXmlSpirit in objXmlItem.Select("spirit"))
                        {
                            ...
                            TreeNode objChild = new TreeNode
                            {
                                Text = objXmlSpirit.SelectSingleNode("name").Value + strSpaceCharacter + '(' +
                                       LanguageManager.GetString("Label_Spirit_Force") + strSpaceCharacter + objXmlSpirit.SelectSingleNode("force").Value + ',' + strSpaceCharacter +
                                       LanguageManager.GetString("Label_Spirit_ServicesOwed") + strSpaceCharacter + objXmlSpirit.SelectSingleNode("services").Value + ')'
                            };
                            objParent.Nodes.Add(objChild);
                            objParent.Expand();
                        }
                        ...
                        foreach (XPathNavigator objXmlLifestyle in objXmlItem.Select("lifestyle"))
                        {
                            ...
                            TreeNode objChild = new TreeNode
                            {
                                Text = (objXmlLifestyle.SelectSingleNode("translate") ?? objXmlLifestyle.SelectSingleNode("baselifestyle")).Value
                                       ...
                                       + strSpaceCharacter + strIncrementString + string.Format(GlobalOptions.CultureInfo, LanguageManager.GetString("Label_LifestylePermanent"), intPermanentAmount.ToString(GlobalOptions.CultureInfo))
                            };
                    ...
                    case "cyberwares":
                        ...
                        foreach (XPathNavigator objXmlCyberware in objXmlItem.Select("cyberware"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(strRating))
                                objChild.Text += strSpaceCharacter + LanguageManager.GetString("String_Rating") + strSpaceCharacter + strRating;
                            ...
                            foreach (XPathNavigator objXmlChild in objXmlCyberware.Select("cyberwares/cyberware"))
                            {
                                ...
                                if (!string.IsNullOrEmpty(strRating))
                                    objChildChild.Text += strSpaceCharacter + LanguageManager.GetString("String_Rating") + strSpaceCharacter + strRating;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            ...
            _xmlBaseChummerNode = XmlManager.Load("packs.xml", _objCharacter.Options.CustomDataDictionary).GetFastNavigator().SelectSingleNode("/chummer");
        ...
        private void WriteGear(XPathNavigator objXmlGear, TreeNode objParent)
        {
            ...
            if (objNode != null)
            {
                ...
                if (!string.IsNullOrEmpty(strExtra))
                    objChild.Text += strSpaceCharacter + LanguageManager.GetString("String_Rating") + strSpaceCharacter + strExtra;
