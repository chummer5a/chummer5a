...
namespace Chummer
{
    public partial class frmDiceRoller : Form
    {
        ...
        public frmDiceRoller(frmChummerMain frmMainForm, ICollection<Quality> lstQualities = null, int intDice = 1)
        {
            ...
            {
                new ListItem("Standard", LanguageManager.GetString("String_DiceRoller_Standard", GlobalOptions.Instance.Language)),
                new ListItem("Large", LanguageManager.GetString("String_DiceRoller_Large", GlobalOptions.Instance.Language)),
                new ListItem("ReallyLarge", LanguageManager.GetString("String_DiceRoller_ReallyLarge", GlobalOptions.Instance.Language))
            };
        ...
        private void cmdRollDice_Click(object sender, EventArgs e)
        {
            ...
            if (chkBubbleDie.Checked)
            {
                if (chkVariableGlitch.Checked || (intGlitchCount == intGlitchThreshold - 1 && (decimal.ToInt32(nudDice.Value) & 1) == 0))
                {
                    ...
                    _lstResults.Add(new ListItem(intBubbleDieResult.ToString(GlobalOptions.Instance.InvariantCultureInfo), LanguageManager.GetString("String_BubbleDie", GlobalOptions.Instance.Language) +
                                                                                                                  strSpaceCharacter + '(' + intBubbleDieResult.ToString(GlobalOptions.Instance.CultureInfo) + ')'));
            ...
            if (intGlitchCount >= intGlitchThreshold)
            {
                if (intHitCount > 0)
                {
                    ...
                    {
                        lblResults.Text += LanguageManager.GetString(intHitCount >= nudThreshold.Value ? "String_DiceRoller_Success" : "String_DiceRoller_Failure", GlobalOptions.Instance.Language) +
                                           strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
                    }
                    else
                        lblResults.Text += string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString());
                }
                else
                    lblResults.Text += LanguageManager.GetString("String_DiceRoller_CriticalGlitch", GlobalOptions.Instance.Language);
            }
            else if (nudThreshold.Value > 0)
            {
                lblResults.Text += LanguageManager.GetString(intHitCount >= nudThreshold.Value ? "String_DiceRoller_Success" : "String_DiceRoller_Failure", GlobalOptions.Instance.Language) +
                                   strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Hits", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
            }
            else
                lblResults.Text += string.Format(LanguageManager.GetString("String_DiceRoller_Hits", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo));

            lblResults.Text += Environment.NewLine + Environment.NewLine + LanguageManager.GetString("Label_DiceRoller_Sum", GlobalOptions.Instance.Language) + strSpaceCharacter + lstRandom.Sum().ToString(GlobalOptions.Instance.CultureInfo);
        ...
        private void cmdReroll_Click(object sender, EventArgs e)
        {
            ...
            if (chkBubbleDie.Checked)
            {
                ...
                {
                    int intBubbleDieResult = 1 + GlobalOptions.Instance.RandomGenerator.NextD6ModuloBiasRemoved();
                    _lstResults.Add(new ListItem(intBubbleDieResult.ToString(), LanguageManager.GetString("String_BubbleDie", GlobalOptions.Instance.Language)
                                                                                + strSpaceCharacter + '(' + intBubbleDieResult.ToString(GlobalOptions.Instance.CultureInfo) + ')'));
                ...
                if (intHitCount > 0)
                {
                    if (nudThreshold.Value > 0)
                    {
                        if (intHitCount >= nudThreshold.Value)
                            lblResults.Text += LanguageManager.GetString("String_DiceRoller_Success", GlobalOptions.Instance.Language) + strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
                        else
                            lblResults.Text += LanguageManager.GetString("String_DiceRoller_Failure", GlobalOptions.Instance.Language) + strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
                    }
                    else
                        lblResults.Text += string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo));
                }
                else
                    lblResults.Text += LanguageManager.GetString("String_DiceRoller_CriticalGlitch", GlobalOptions.Instance.Language);
            }
            ...
            {
                if (intHitCount >= nudThreshold.Value)
                    lblResults.Text += LanguageManager.GetString("String_DiceRoller_Success", GlobalOptions.Instance.Language) + strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Hits", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
                else
                    lblResults.Text += LanguageManager.GetString("String_DiceRoller_Failure", GlobalOptions.Instance.Language) + strSpaceCharacter + '(' + string.Format(LanguageManager.GetString("String_DiceRoller_Glitch", GlobalOptions.Instance.Language), intHitCount.ToString(GlobalOptions.Instance.CultureInfo)) + ')';
            }
            else
            ...
            lblResults.Text += Environment.NewLine + Environment.NewLine + LanguageManager.GetString("Label_DiceRoller_Sum", GlobalOptions.Instance.Language) + strSpaceCharacter + (lstRandom.Sum() + intKeepSum).ToString(GlobalOptions.Instance.CultureInfo);
            lstResults.BeginUpdate();
            lstResults.DataSource = null;
            lstResults.ValueMember = "Value";
            lstResults.DisplayMember = "Name";
