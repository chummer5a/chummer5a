...
namespace Chummer
{
    public partial class frmCreateSpell : Form
    {
        ...
        public frmCreateSpell(Character objCharacter)
        {
            ...
            _objXmlDocument = XmlManager.Load("spells.xml", objCharacter.Options.CustomDataDictionary);
        }
        ...
        private void ChangeModifiers()
        {
            ...
            foreach (CheckBox chkCheckbox in flpModifiers.Controls.OfType<CheckBox>())
            {
                if (!string.IsNullOrEmpty(chkCheckbox.Text))
                {
                    ...
                    chkCheckbox.Text += LanguageManager.GetString("String_Space", GlobalOptions.Language) + '(' + chkCheckbox.Tag + ')';
                }
            }
            foreach (Panel panChild in flpModifiers.Controls.OfType<Panel>())
            {
        ...
        private void AcceptForm()
        {
            ...
            if (!string.IsNullOrEmpty(strMessage))
            {
                Program.MainForm.ShowMessageBox(strMessage, LanguageManager.GetString("Title_CreateSpell", GlobalOptions.Language), MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
