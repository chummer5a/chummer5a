            ...
            if (xmlVehicleList?.Count > 0)
            {
                foreach (XmlNode objXmlGear in xmlVehicleList)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            string _ = objTemp.TotalAvail(GlobalOptions.Instance.CultureInfo, GlobalOptions.Instance.Language);
                        }
                        catch (Exception e)
                        {
                            if (_blnAddExceptionInfoToErrors)
            ...
            if (xmlWeaponList?.Count > 0)
            {
                foreach (XmlNode objXmlGear in xmlWeaponList)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            string _ = objTemp.TotalAP(GlobalOptions.Instance.Language);
                        }
                        catch (Exception e)
                        {
                            if (_blnAddExceptionInfoToErrors)
                        ...
                        try
                        {
                            string _ = objTemp.CalculatedAmmo(GlobalOptions.Instance.CultureInfo, GlobalOptions.Instance.Language);
                        }
                        catch (Exception e)
                        {
                            if (_blnAddExceptionInfoToErrors)
                        ...
                        try
                        {
                            string _ = objTemp.CalculatedDamage(GlobalOptions.Instance.CultureInfo, GlobalOptions.Instance.Language);
                        }
                        catch (Exception e)
                        {
                            if (_blnAddExceptionInfoToErrors)
