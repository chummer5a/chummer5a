...
namespace Chummer.UI.Shared
{
    public partial class VehiclesTabUserControl : UserControl
    {
        ...
        private void cmdVehicleGearReduceQty_Click(object sender, EventArgs e)
        {
            ...
            if (!_objCharacter.ConfirmDelete(LanguageManager.GetString("Message_ReduceQty", GlobalOptions.Language).Replace("{0}", decSelectedValue.ToString(GlobalOptions.Instance.CultureInfo))))
                return;
        ...
        private void RefreshSelectedVehicle()
        {
            ...
            if (treVehicles.SelectedNode?.Tag is Vehicle objVehicle)
            {
                ...
                if (objVehicle.IsDrone && _objCharacter.Options.Dronemods)
                {
                ...
                {
                    DisplayVehicleDroneMods(objVehicle.IsDrone && _objCharacter.Options.Dronemods);
                    DisplayVehicleMods(!(objVehicle.IsDrone && _objCharacter.Options.Dronemods));
                }
                else
                {
            ...
            else if (treVehicles.SelectedNode?.Tag is Weapon objWeapon)
            {
                ...
                lblVehicleWeaponDamage.Text =
                    objWeapon.CalculatedDamage(GlobalOptions.Instance.CultureInfo, GlobalOptions.Language);
                lblVehicleWeaponAccuracy.Text =
                    objWeapon.DisplayAccuracy(GlobalOptions.Instance.CultureInfo, GlobalOptions.Language);
                ...
                if (objWeapon.WeaponType == "Ranged" || (objWeapon.WeaponType == "Melee" && objWeapon.Ammo != "0"))
                {
                    lblVehicleWeaponAmmo.Text =
                        objWeapon.CalculatedAmmo(GlobalOptions.Instance.CultureInfo, GlobalOptions.Language);
            ...
            else if (treVehicles.SelectedNode?.Tag is Gear objGear)
            {
                ...
                lblVehicleSlotsLabel.Visible = true;
                lblVehicleSlots.Visible = true;
                lblVehicleSlots.Text = objGear.CalculatedCapacity + LanguageManager.GetString("String_Space", GlobalOptions.Language) + '(' + objGear.CapacityRemaining.ToString("#,0.##", GlobalOptions.Instance.CultureInfo) + LanguageManager.GetString("String_Space", GlobalOptions.Language) + LanguageManager.GetString("String_Remaining", GlobalOptions.Language) + ')';
