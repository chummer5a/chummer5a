...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:template name="skills1">
    <xsl:variable name="skillcut" select="round(count(skills/skill[knowledge = 'False' and (rating &gt; 0)]) div 2)"/>
        ...
        <xsl:when test="position() = 1">
          <tr>
            <td colspan="6" style="border-bottom:solid black 1px;">
              <strong>
                <xsl:value-of select="skillcategory" /> Compétences
              </strong>
            </td>
          </tr>
        </xsl:when>
  ...
  <xsl:template name="skills3">
    <xsl:for-each select="skills/skill[knowledge = 'True' and islanguage = 'True']">
      ...
      <tr>
        ...
        <td width="45%" valign="top">
          <xsl:if test="islanguage = 'True'">Langage: </xsl:if>
          <xsl:value-of select="name" />
          <xsl:if test="spec != ''">
            (<xsl:value-of select="spec" />)
          </xsl:if>
